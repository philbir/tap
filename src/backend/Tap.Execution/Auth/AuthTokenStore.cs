...
/// <summary>Where the token cache lives. A record rather than a bare string so the Studio and
/// the CLI can be seen to be pointing at the same folder.</summary>
public sealed record AuthTokenStoreOptions
{
    /// <summary>Directory holding <c>auth-tokens.json</c> — conventionally <c>~/.tap</c>.</summary>
...
public sealed class AuthTokenStore
{
    ...
    public AuthTokenStore(AuthTokenStoreOptions options, ILogger<AuthTokenStore> logger)
    {
        _logger = logger;
