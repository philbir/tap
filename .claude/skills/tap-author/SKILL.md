...
```
tap.md                                ← kind: workspace — the root marker + manifest
collections/<slug>/_collection.md     ← kind: collection — baseUrl, default headers/auth, stages, vars
collections/<slug>/**/*.req.md        ← kind: request — one executable request each
collections/<slug>/**/*.auth.md      ← collection-scoped auth profile (sees collection+stage vars)
auth/*.auth.md                        ← workspace-scoped auth profiles (shared, see only workspace+env vars)
environments/*.env.md                 ← kind: env — named variable sets (local / dev / prod)
tests/*.test.md                       ← kind: test — named group of checks over requests/flows
tests/*.flow.md                       ← kind: flow — multi-step sequence with value extraction
```

Nested folders under a collection are pure grouping — every request belongs to exactly
one collection (the nearest `_collection.md` above it), and inherits its `baseUrl`,
...
workspace `tap.md` vars → owning collection vars → active stage vars → active env vars →
...
**Agent access.** `agent: false` on a `_collection.md` fences that collection off from
...
write the `.req.md` with assertions pinned to what you observed, then `send` the saved
