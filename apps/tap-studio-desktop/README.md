...
`src/Tap.Studio/Tap.Studio.csproj`), so the webview is single-origin with the
API and `fetch('/api/...')` Just Works without any URL plumbing.

## Layout

```
apps/tap-studio-desktop/
...
```bash
node apps/tap-studio-desktop/scripts/compile-server.mjs <triple>
yarn --cwd apps/tap-studio-desktop tauri build --target <triple> --bundles ...
```
...
Output bundles land under `apps/tap-studio-desktop/src-tauri/target/<triple>/release/bundle/`.
...
# Public key: paste the contents of ~/.tauri/tap-studio.key.pub into
# src-tauri/tauri.conf.json's plugins.updater.pubkey (currently REPLACE_ME).
# Private key: contents of ~/.tauri/tap-studio.key → TAURI_SIGNING_PRIVATE_KEY
# Password (if you set one): TAURI_SIGNING_PRIVATE_KEY_PASSWORD
```
