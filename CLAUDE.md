...
- **`Tap.Inspector.Mcp` is the inspector's shared MCP tool layer** (`TapInspectorTools` +
  `IMcpCaptureProvider`) — the same served-twice pattern as `Tap.Studio.Mcp`, for the *other*
  product. `tap mcp` (in `Tap.Cli`) hosts it over stdio with an HTTP-backed provider; `/mcp` in
  `Tap.Server` hosts it in-process over the live ring (`ModelContextProtocol.AspNetCore`). It references **only `Tap.Core`** and
  the `ModelContextProtocol` core package — never `Tap.Server`, which references it. Tool
  contracts must not fork: change them here, never per host.
- **Inspector agent surface** — off by default (`Inspector:Agent:Enabled`, or
  `.WithAgentAccess()` in an AppHost). Layering: `CaptureRedactor` + the `Captured*` DTOs live in
  `Tap.Core/{Redaction,Capture}/`; the `RequestRecord` → `Captured*` projection lives in
  `Tap.Server/Agent/CaptureProjection.cs`; `/api/agent/*` serves it. **Redaction happens in the
  inspector process, at read time, and nowhere else** — the ring keeps raw bytes so the UI still
  shows real values, and `tap mcp` is a client of the already-redacted REST surface rather than
  a second redactor. Three rules hold: fail closed on unknown content types, describe rather than
  blank (a JWT keeps its claims, loses its signature), and hand out salted fingerprints instead
  of values. There is **no reveal path** — do not add one. Adding a property to `RequestRecord`
  fails `CaptureProjectionTests` until it is mapped or explicitly withheld; that guardrail is the
  point, so map or withhold rather than editing the test. `/api/agent/*` and `/mcp` sit behind
  `AgentGate`: **loopback-only regardless of `Inspector:UiHost`**, plus this run's token from the
  `0600` handle at `~/.tap/inspector/<uiPort>.json` (`AgentBridgeFile`, which `tap mcp` discovers).
...
- **`Tap.Hosting`** (library, namespace `Aspire.Hosting`) — extension methods consumers call from their own AppHost. Primary surface: `AddTap<T>`, `AddTapContainer`, `WithTap`, `tap.WithTunnel(name, configure)`, `tap.WithQuickTunnel`, `tap.WithTailscaleFunnel`, `WithExistingTunnel`, `WithApiManagedTunnel`, `WithDynamicHostname`, `WithSystemDaemon`/`WithEphemeralDaemon`/`WithFunnelPort` (Tailscale), plus the Studio pair `AddTapStudio<T>` (project) and `AddTapStudioContainer` (the `ghcr.io/philbir/tap-studio` image). Low-level escape hatches still public: `AddCloudflaredTunnel`, `AddTailscaleFunnel`, `WithCloudflareTunnel`, `WithTailscaleFunnel(target, tunnel)`. No runtime; pure AppHost wiring.
- **Two container images**, both built by `.github/workflows/docker-publish.yml` on a tag: `ghcr.io/philbir/tap` from `src/backend/Tap.Server/Dockerfile` (Tunnels inspector) and `ghcr.io/philbir/tap-studio` from `src/backend/Tap.Studio/Dockerfile` (Studio: UI + API + engine, workspace bind-mounted at `/workspace`, state at `/state`, port 8080). Both Dockerfiles take the repo root as build context and plug a pre-built Vite `dist/` into `wwwroot/` so the SDK layer never runs yarn. Adding a third image means adding a row to that workflow's `image` matrix in both jobs — the artifact name and cache scope are keyed on `image.key`.
...
- **`Tap.Server`** (`Microsoft.NET.Sdk.Web`) — standalone ASP.NET Core app: YARP reverse proxy + capture middleware + SSE feed + bundled React UI in `wwwroot`. Reads its config from `Inspector:*` and `Cloudflare:*` env vars set by the AppHost. Note the UI port ends in a SPA fallback, so an *unmapped* `/api/*` path returns `index.html` with a 200 — API routes that need to say "no" must be mapped and answer explicitly.
