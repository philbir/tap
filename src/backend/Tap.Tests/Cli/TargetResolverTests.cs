...
public class TargetResolverTests
{
    ...
    {
        var ws = Workspace(Set("tests/orders.test.tap", "Order API"));
        Assert.True(TargetResolver.TryResolve(ws, "tests/orders.test.tap", Runnable, out var target, out _));
        Assert.Equal("tests/orders.test.tap", target.Path);
    }
    ...
    public void Backslashes_in_a_path_are_accepted()
    {
        ...
        var ws = Workspace(Set("tests/orders.test.tap", "Order API"));
        Assert.True(TargetResolver.TryResolve(ws, @"tests\orders.test.tap", Runnable, out _, out _));
    }

    [Fact]
    public void Flows_resolve_alongside_test_sets()
    {
        var ws = Workspace(Set("tests/orders.test.tap", "Order API"), Flow("tests/checkout.flow.tap", "Checkout"));
        Assert.True(TargetResolver.TryResolve(ws, "Checkout", Runnable, out var target, out _));
        Assert.Equal("tests/checkout.flow.tap", target.Path);
    }
    ...
    {
        var ws = Workspace(Set("tests/a.test.tap", "Smoke"), Set("tests/b.test.tap", "Smoke"));
        ...
        Assert.Contains("tests/a.test.tap", error, StringComparison.Ordinal);
        Assert.Contains("tests/b.test.tap", error, StringComparison.Ordinal);
    }
    ...
    {
        var ws = Workspace(Set("tests/a.test.tap", "Smoke"), Set("tests/b.test.tap", "Smoke"));
        Assert.True(TargetResolver.TryResolve(ws, "tests/a.test.tap", Runnable, out var target, out _));
        Assert.Equal("tests/a.test.tap", target.Path);
    }
    ...
    [Theory]
    [InlineData("tests/orders.test.tap", "orders")]
    [InlineData("tests/checkout.flow.tap", "checkout")]
    [InlineData("collections/demo/get.req.tap", "get")]
    // A single-extension name keeps everything before its one dot.
    [InlineData("workspace.tap", "workspace")]
