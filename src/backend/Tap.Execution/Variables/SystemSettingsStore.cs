...
/// <summary>Where the user-level settings file lives and what to seed it with. Replaces the
/// web host's options object so a CLI can point at the same <c>~/.tap</c> without dragging
/// Kestrel's configuration in with it.</summary>
public sealed record SystemSettingsOptions
{
    /// <summary>Directory holding <c>system.json</c>. Defaults to <c>~/.tap</c>, honouring
    /// <c>TAP_SYSTEM_DIR</c> — the same resolution the Studio applies, so both see one file.</summary>
    public required string SystemDir { get; init; }

    /// <summary>Provider configs used to seed the file on first creation only. After that the
...
public sealed class SystemSettingsStore
{
    ...
    public SystemSettingsStore(SystemSettingsOptions options, ILogger<SystemSettingsStore> logger)
    {
        _logger = logger;
